...
public:
  ...
  [[nodiscard]] SiblingNodes siblings() && noexcept
  {
    return std::move( siblings_ );
  }
};
...
public:
  /// @throw error::RuntimeError If the type of `data` does not match the value
  /// of `expr_type`.
  template<typename T>
  ...
  [[nodiscard]] types::TokenT token() &&
  {
    return std::move( std::get<types::TokenT>( expr_ ) );
  }
  const types::TokenT& token() const&
  {
    return std::get<types::TokenT>( expr_ );
  }
  ...
  [[nodiscard]] types::EvalT value() const
  {
    return std::get<types::EvalT>( expr_ );
  }
