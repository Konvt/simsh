    ...
    switch ( state ) {
      ...
      case StateType::INDIGIT: {
        if ( character == '>' ) // get (\d+)>, expecting '&' or '>'
          state = StateType::INRARR;
                    ...
                    character ) ) { // get (\d+), then expecting CMD characters
          save_char = ( discard_char = false );
          state = StateType::INCMD;
        }
      } break;
      ...
      case StateType::INAND: {
        if ( character == '&' ) { // get &&, done
          ...
          state
            = StateType::INMEG_OUTPUT; // get &>, still expecting '>' or nothing
        else {
      ...
      default: {
        state = StateType::DONE;
        token_type = types::TokenType::ERROR;
        throw error::ArgumentError( "tokenizer"sv,
                                    "the state machine status is incorrect"sv );
      } break;
    }
