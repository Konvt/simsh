...
Parser::StmtNodePtr Parser::statement()
{
  switch ( const auto tkn_tp = tknizr_.peek().type_; tkn_tp ) {
    ...
    case types::TokenType::NEWLINE: {
      ...
      return make_unique<ExprNode>( types::ExprKind::value,
                                    constants::EvalSuccess );
    }
...
Parser::StmtNodePtr Parser::inner_statement_extension(
  Parser::StmtNodePtr left_stmt )
{
  switch ( tknizr_.peek().type_ ) {
    case types::TokenType::AND: { // connector
      tknizr_.consume( types::TokenType::AND );
...
Parser::StmtNodePtr Parser::redirection( Parser::StmtNodePtr left_stmt )
{
  ...
  return make_unique<StmtNode>(
    stmt_kind, move( left_stmt ), nullptr, move( arguments ) );
}
...
Parser::StmtNodePtr Parser::output_redirection( Parser::StmtNodePtr left_stmt )
{
  ...
  auto extract_params = [this]( StmtNode::SiblingNodes& args,
    ...
    auto [match_result, matches]
      = utils::match_string( tknizr_.consume( expecting ), re_str );

    assert( match_result == true );
    if ( expecting == types::TokenType::MERG_STREAM ) {
      for ( size_t i = 1; i <= 2; ++i ) {
        if ( auto match_str = matches[i].str(); match_str.empty() )
          args.emplace_back( make_unique<ExprNode>( types::ExprKind::value,
      ...
      if ( auto match_str = matches[1].str(); match_str.empty() )
        ...
        args.emplace_back(
          make_unique<ExprNode>( types::ExprKind::value, stoi( match_str ) ) );
    }
  };
  ...
  switch ( tknizr_.peek().type_ ) {
    case types::TokenType::OVR_REDIR: { // >
      ...
      extract_params(
        arguments, redirection_regex, types::TokenType::OVR_REDIR );
      assert( arguments.size() == 1 );
    } break;
    case types::TokenType::APND_REDIR: { // >>
      stmt_kind = types::StmtKind::appnd_redrct;
      extract_params(
        arguments, redirection_regex, types::TokenType::APND_REDIR );
      assert( arguments.size() == 1 );
    } break;
    case types::TokenType::MERG_OUTPUT: { // &>
  ...
  // child node.
  if ( tknizr_.peek().is(
         types::TokenType::MERG_STREAM ) ) { // output_redirecti
    StmtNode::SiblingNodes subargs;
    extract_params(
      subargs, combined_redir_regex, types::TokenType::MERG_STREAM );
...
Parser::StmtNodePtr Parser::combined_redirection_extension(
  Parser::StmtNodePtr left_stmt )
{
  ...
  extract_params(
    arguments, combined_redir_regex, types::TokenType::MERG_STREAM );
  ...
  switch ( tknizr_.peek().type_ ) {
    ...
    case types::TokenType::APND_REDIR: { // >>
      StmtNode::SiblingNodes subargs;
      extract_params(
        subargs, redirection_regex, types::TokenType::APND_REDIR );
      assert( subargs.size() == 1 );
      subargs.emplace_back( expression() );
...
Parser::StmtNodePtr Parser::logical_not()
{
  ...
  if ( tknizr_.peek().is( types::TokenType::CMD )
    ...
    tknizr_.consume( types::TokenType::LPAREN );
    return make_unique<StmtNode>( types::StmtKind::logical_not,
                                  inner_statement() );
...
Parser::ExprNodePtr Parser::expression()
{
  ...
  const auto token_str = tknizr_.consume( token_type == types::TokenType::CMD
                                            ? types::TokenType::CMD
                                            : types::TokenType::STR );
  ...
  while ( tknizr_.peek().is( types::TokenType::CMD )
    ...
    arguments.emplace_back( make_unique<ExprNode>(
      tkn_tp == types::TokenType::CMD ? types::ExprKind::command
                                      : types::ExprKind::string,
      tknizr_.consume( tknizr_.peek().type_ ) ) );
  }

  return make_unique<ExprNode>( arguments.empty()
