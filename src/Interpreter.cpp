...
types::EvalT Interpreter::pipeline_stmt( StmtNodeT pipeline_stmt ) const
{
  ...
  assert( pipeline_stmt->left() != nullptr
          && pipeline_stmt->right() != nullptr );
...
types::EvalT Interpreter::output_redirection( StmtNodeT oup_redr ) const
{
   ...
   * `types::ExprKind::value`, which specifies the destination file descriptor.
   */
  types::FDType file_d = STDOUT_FILENO;
  if ( oup_redr->type() == types::StmtKind::appnd_redrct
       || oup_redr->type() == types::StmtKind::ovrwrit_redrct ) {
    ExprNodeT arg_node
      = static_cast<ExprNode*>( oup_redr->siblings().front().get() );

    assert( arg_node->kind() == types::ExprKind::value );

    file_d = arg_node->value() == constants::InvalidValue ? STDOUT_FILENO
                                                          : arg_node->value();
  }
  ...
  if ( oup_redr->left() != nullptr
       && oup_redr->left()->type() == types::StmtKind::trivial )
    return pguard.exit_code() == constants::ExecSuccess;
  else
    return pguard.exit_code();
}
...
types::EvalT Interpreter::merge_stream( StmtNodeT merg_redr ) const
{
   ...
   * descriptor. */
  ExprNodeT arg_node1
    = static_cast<ExprNode*>( merg_redr->siblings()[0].get() );
  ExprNodeT arg_node2
    = static_cast<ExprNode*>( merg_redr->siblings()[1].get() );

  assert( arg_node1->kind() == types::ExprKind::value );
  assert( arg_node2->kind() == types::ExprKind::value );

  const auto l_fd = arg_node1->value() == constants::InvalidValue
                      ? STDERR_FILENO
                      : arg_node1->value();
  const auto r_fd = arg_node2->value() == constants::InvalidValue
                      ? STDOUT_FILENO
                      : arg_node2->value();
...
types::EvalT Interpreter::input_redirection( StmtNodeT inp_redr ) const
{
  ...
  if ( inp_redr->siblings().size() != 1 ) {
    iout::logger << error::ArgumentError( "input redirection"sv,
                                          "argument number error"sv );
    return !constants::EvalSuccess;
  }

  const auto& filename
    = static_cast<const ExprNode*>( inp_redr->siblings().front().get() )
        ->token();
...
types::EvalT Interpreter::builtin_exec( ExprNodeT expr ) const
{
  ...
  switch ( expr->token().front() ) {
    case 'c': { // cd
      ...
      assert(
        static_cast<const ExprNode*>( expr->siblings().front().get() )->kind()
        != types::ExprKind::value );

      types::StrViewT target_dir
        = expr->siblings().size() == 0
            ? utils::get_homedir()
            : static_cast<const ExprNode*>( expr->siblings().front().get() )
                ->token();
      ...
      try {
        ...
        iout::logger.print(
          error::SystemCallError( format( "cd: {}", target_dir.data() ) ) );
        return !constants::EvalSuccess;
      }
      return constants::EvalSuccess;
    } break;
    ...
    case 'e': { // exit or exec
      if ( expr->token() == "exit" ) {
        ...
        const auto error_info
          = static_cast<ExprNode*>( expr->siblings().front().get() );
        iout::logger << error::ArgumentError(
  ...
  if ( pguard.is_child() ) {
    ...
    ranges::transform( expr->siblings(),
                       ...
                       []( const auto& sblng ) -> char* {
                         assert( sblng->type() == types::StmtKind::trivial );
                         ExprNodeT arg_node
                           = static_cast<ExprNode*>( sblng.get() );
                         assert( arg_node->kind() != types::ExprKind::value );
    ...
    if ( pipe.reader().pop<bool>() )
      iout::logger << error::ArgumentError( expr->token(),
                                            "command not found" );
