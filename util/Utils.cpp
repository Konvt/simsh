...
vector<types::StringT> get_envpath()
{
  ...
  ranges::transform(
    envpath | views::split( ':' ), back_inserter( ret ), []( auto&& path ) {
...
types::StringT tilde_expansion( const types::StringT& token )
{
  if ( regex_search( token, regex( "^~(/.*)?$" ) ) )
    return format( "{}{}",
                   get_homedir(),
                   types::StrViewT( token.begin() + 1, token.end() ) );
  return {};
}

pair<bool, smatch> match_string( const types::StringT& str,
