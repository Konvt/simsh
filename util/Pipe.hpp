...
class PipeReader : public Pipe {
  ...
  static constexpr bool is_valid_type_v
    = std::conjunction_v<std::negation<std::is_void<std::remove_pointer_t<
                           std::decay_t<T>>>>, // forbidden any void pointers
...
public:
  ...
  template<typename T>
    requires std::conjunction_v<
      ...
      std::negation<std::is_void<
        std::remove_pointer_t<std::decay_t<T>>>> // forbidden any void pointers
      >
  ...
  void push( const T<U, Args...>& value ) const
  {
    if ( !writer_closed_ )
      std::ranges::for_each(
        value, [this]( const auto& ele ) -> void { this->push( ele ); } );
  }
